...
class ImageReader():
    def __init__(self):
        #self._local_costmap_sub = rospy.Subscriber("/map", OccupancyGrid, self.grid_cb)
        self.bridge = CvBridge()
        self.grid_flipped = None
        self.origin_x = 0
        self.origin_y = 0
        self.grid_res = 0
...
class LaneKeeping():
    ...
    def robot_position(self):
        ...
        global count
        if count == 0:
            while self.imagereader.grid_res == 0:
                pass
        '''
        ...
        if self.imagereader.grid_res !=0:
            count = 1
            prev_grid_res = self.imagereader.grid_res
            prev_grid_flipped = self.imagereader.grid_flipped
            prev_origin_y = self.imagereader.origin_x
            prev_origin_x = self.imagereader.origin_y 

        if self.imagereader.grid_res == 0:
            self.imagereader.grid_res = prev_grid_res
            self.imagereader.grid_flipped = prev_grid_flipped
            self.imagereader.origin_x = prev_origin_y
            self.imagereader.origin_y = prev_origin_x
        ...
        print(self.imagereader.grid_flipped)

        grid_trimmed = self.imagereader.grid_flipped[robot_y_centre-(lane_width/2):robot_y_centre+(lane_width/2), :]
